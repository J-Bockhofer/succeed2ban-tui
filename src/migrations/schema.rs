...
mod test {
    ...
    pub fn test_insert() -> Result<()>{
        let conn = Connection::open("test.db")?;
        let _ = country::insert_new_country(&conn, "Doitschland", Some("DE"), Some(0), Some(0)).expect("Country insertion failed");
        let _ = region::insert_new_region(&conn, "Undetussen", "Doitschland", Some(0), Some(0)).expect("Region insertion failed");
        let _ = city::insert_new_city(&conn, "Humburg", "Doitschland", "Undetussen",Some(0), Some(0)).expect("City insertion failed");
        let _ = isp::insert_new_ISP(&conn,"Telecum", Some(0), Some(0)).expect("ISP insertion failed");
    ...
    pub fn test_get_timestamps_by_country() -> Result<()> {
        let conn = Connection::open("test.db")?;
        let msgs = message::get_message_timestamps_by_country(&conn, "Doitschland").unwrap();
        let mut res: String = "".to_string();
        if !msgs.is_empty() {
            // stuff in vec
            res = msgs[0].clone();
        }
        assert_eq!(res, "2022-03-11 23:45:31:512".to_string());
        Ok(())
    }    
    #[test]
