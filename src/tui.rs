...
/*     // FILEREADER
    let _cancellation_token2 = self.cancellation_token.clone();
    let _event_tx2 = self.event_tx.clone();
    let _event_tx3 = self.event_tx.clone();
    ...
    handle.spawn(async move {
      ...
      let _resp = tasks::notify_change(&path, _event_tx2).await.unwrap_or_else(|err| {
          _event_tx3.send(Event::Error).unwrap();
      });
    }); */
